  ...
  (unwind-protect
      (persp-test-with-persp
        ...
        (let ((dummy-buffer (get-buffer-create "*dummy*"))
          ...
          (should-not (buffer-live-p (get-buffer "*scratch* (B)")))
...
(ert-deftest state-save-and-load ()
  (unwind-protect
      (persp-test-with-persp
        ...
        (when (get-buffer "*scratch* (B)") (kill-buffer "*scratch* (B)"))
...
(ert-deftest state-load-lazy-creates-scratch-on-demand ()
  (unwind-protect
      (persp-test-with-persp
        (persp-switch "A")
...
(ert-deftest state-kill-buffer-does-not-load-lazy-perspective ()
  (unwind-protect
      (persp-test-with-persp
        (persp-test-with-files nil (SHARED A-ONLY B-ONLY)
          (persp-switch "A")
          (switch-to-buffer SHARED)
          (switch-to-buffer A-ONLY)
          (persp-switch "B")
          (switch-to-buffer SHARED)
          (switch-to-buffer B-ONLY)
          (persp-state-save "state-1.el")
          (persp-mode -1)
          (delete-other-windows)
          (when (get-buffer "*scratch* (A)") (kill-buffer "*scratch* (A)"))
          ...
          (should-not (gethash "A" (frame-parameter nil 'persp--state-lazy-pending)))
          (should (gethash "B" (frame-parameter nil 'persp--state-lazy-pending)))
          (should (kill-buffer SHARED))
          (should-not (get-buffer "SHARED"))
          (should (gethash "B" (frame-parameter nil 'persp--state-lazy-pending)))
          (should-not (buffer-live-p (get-buffer "*scratch* (B)")))
          (should (equal (list "B-ONLY")
                         (persp--state-single-buffers
                          (gethash "B" (frame-parameter nil 'persp--state-lazy-pending)))))
          (persp-switch "B")
          (should-not (get-buffer "SHARED"))
          (should (equal (list "B-ONLY")
                         (sort (mapcar #'buffer-name (persp-test-buffer-list (persp-curr)))
                               #'string-lessp)))))
    (persp-test-clean-files "SHARED" "A-ONLY" "B-ONLY" "state-1.el")))

(ert-deftest state-kill-buffer-keeps-lazy-last-buffer-perspective-deferred ()
  (unwind-protect
      (persp-test-with-persp
        (persp-test-with-files nil (SHARED A-ONLY)
          (persp-switch "A")
          (switch-to-buffer SHARED)
          (switch-to-buffer A-ONLY)
          (persp-switch "B")
          ...
          (persp-switch "A")
          (should (gethash "B" (frame-parameter nil 'persp--state-lazy-pending)))
          (should-not (kill-buffer SHARED))
          (should (get-buffer "SHARED"))
          (should (gethash "B" (frame-parameter nil 'persp--state-lazy-pending)))
          (should-not (buffer-live-p (get-buffer "*scratch* (B)")))
          (should (equal (list "SHARED")
                         (persp--state-single-buffers
                          (gethash "B" (frame-parameter nil 'persp--state-lazy-pending)))))))
    (persp-test-clean-files "SHARED" "A-ONLY" "state-1.el")))
...
(ert-deftest state-save-excludes-named-perspectives ()
  (unwind-protect
      (persp-test-with-persp
        (persp-test-with-files nil (MAIN A1 B1)
          (persp-test-clean-files "state-exclude-named.el")
          (switch-to-buffer MAIN)
          (persp-switch "A")
          (persp-set-buffer A1)
          (persp-switch "B")
          (persp-set-buffer B1)
          (persp-state-save "state-exclude-named.el" nil '("A"))
          (let* ((state (persp--state-read-file "state-exclude-named.el"))
                 (frame (car (persp--state-complete-frames state)))
                 (persps (persp--state-frame-v2-persps frame))
                 (order (persp--state-frame-v2-order frame))
                 (files (sort (copy-sequence (persp--state-complete-files state))
                              #'string-lessp)))
            (should-not (gethash "A" persps))
            (should (equal (sort (copy-sequence order) #'string-lessp)
                           '("B" "main")))
            (should-not (member (expand-file-name "A1") files))
            (should (member (expand-file-name "B1") files))
            (should (member (expand-file-name "MAIN") files))))
        (persp-mode -1)
        (delete-other-windows)
        (persp-mode 1)
        (persp-state-load "state-exclude-named.el")
        (should-not (member "A" (persp-names)))
        (should (member "B" (persp-names)))
        (persp-switch "B")
        (should (equal (list "B1")
                       (sort (mapcar #'buffer-name (persp-test-buffer-list (persp-curr)))
                             #'string-lessp))))
    (persp-test-clean-files "MAIN" "A1" "B1" "state-exclude-named.el")))

(ert-deftest state-save-excludes-merge-metadata-but-keeps-buffer-membership ()
  (let ((persp-merge-list nil))
    (unwind-protect
        (persp-test-with-persp
          (persp-test-with-files nil (A1 B1)
            (persp-test-clean-files "state-exclude-merge.el")
            (persp-switch "A")
            (persp-set-buffer A1)
            (persp-switch "B")
            (persp-set-buffer B1)
            (persp-merge "B" "A")
            (persp-state-save "state-exclude-merge.el" nil '("A"))
            (let* ((state (persp--state-read-file "state-exclude-merge.el"))
                   (frame (car (persp--state-complete-frames state)))
                   (b-state (gethash "B" (persp--state-frame-v2-persps frame))))
              (should (equal nil (persp--state-frame-v2-merge-list frame)))
              (should (equal (list "A1" "B1")
                             (sort (copy-sequence (persp--state-single-buffers b-state))
                                   #'string-lessp)))))
          (persp-mode -1)
          (delete-other-windows)
          (when (get-buffer "*scratch* (A)") (kill-buffer "*scratch* (A)"))
          (when (get-buffer "*scratch* (B)") (kill-buffer "*scratch* (B)"))
          (persp-mode 1)
          (persp-state-load "state-exclude-merge.el")
          (should-not (member "A" (persp-names)))
          (persp-switch "B")
          (should (equal (list "A1" "B1")
                         (sort (mapcar #'buffer-name (persp-test-buffer-list (persp-curr)))
                               #'string-lessp))))
      (persp-test-clean-files "A1" "B1" "state-exclude-merge.el"))))

(ert-deftest state-save-excluding-main-skips-purge ()
  (unwind-protect
      (persp-test-with-persp
        (persp-test-with-files nil (MAIN)
          (persp-test-clean-files "state-exclude-main.el")
          (switch-to-buffer MAIN)
          (let ((persp-purge-initial-persp-on-save t)
                (persp-state-excluded-perspective-names '("main")))
            (persp-state-save "state-exclude-main.el")
            (should (buffer-live-p MAIN))
            (let ((state (persp--state-read-file "state-exclude-main.el")))
              (should-not (persp--state-complete-frames state))
              (should-not (persp--state-complete-files state))))))
    (persp-test-clean-files "MAIN" "state-exclude-main.el")))
