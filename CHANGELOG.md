...
- `persp-state-load`: non-interactive calls now fall back to `persp-state-default-file`, and an optional `NOERROR` argument can enable `persp-mode` without error when the target state file does not exist.
- `kill-buffer` and `vc-follow-link`: avoid materializing deferred perspectives while cleaning up shared buffer membership.
